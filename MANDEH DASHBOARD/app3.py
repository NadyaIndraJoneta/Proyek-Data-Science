...
# ============================================================
# CUSTOM STYLE (BEDAKAN SIDEBAR VS HALAMAN UTAMA + CARD SHADING)
# ============================================================
st.markdown(
    """
    <style>
    /* =========================
       WARNA HALAMAN & SIDEBAR
       ========================= */
    .stApp {
    ...
    /* =========================
       CARD UNIVERSAL UNTUK CHART/TABEL
       ========================= */
    .card {
        background: linear-gradient(180deg, #0f1a2b, #0b1220);
        padding: 16px;
    ...
    .header-card {
        ...
        background: linear-gradient(180deg, #101c33, #0b1220);
        padding: 16px 28px;
        border-radius: 18px;
...
def style_plotly(fig):
    """Biar chart menyatu dengan background card (tidak terlihat putih)."""
    fig.update_layout(
...
with header_l:
    if safe_exists(LOGO_PATH):
        st.markdown(
            """
            <div style="
                margin-left:-90px;   /* makin minus = makin ke kiri */
                margin-top:-25px;    /* makin minus = makin ke atas */
            ">
            """,
            unsafe_allow_html=True
        )
    ...
    st.markdown(
        ...
        <div class="header-wrapper">
            <div class="header-card">
                <h1>Dashboard Prediksi Kunjungan Wisata Mandeh</h1>
                <h3>Kabupaten Pesisir Selatan</h3>
            </div>
        </div>
        """,
        unsafe_allow_html=True
...
if page == "Home":
    ...
    # Line chart (card)
    with col1:
        card_open("📈 Total Penumpang Harian")
    ...
    # Bar chart (card)
    with col2:
        card_open("🧭 Rute Paling Ramai (Top 10)")
        top_rute = (
    ...
    # Donut (card)
    with col3:
        card_open("📊 Distribusi Penumpang Berdasarkan Rute")
        pie_rute = (
            df_f.groupby("rute", as_index=False)["jumlah_penumpang"].sum()
            .sort_values("jumlah_penumpang", ascending=False)
        )
        ...
        fig_pie = px.pie(pie_show, names="rute", values="jumlah_penumpang", hole=0.45)
        fig_pie.update_traces(textinfo="percent+label", textposition="inside")
        fig_pie.update_layout(height=420, legend=dict(orientation="v", x=1.02, y=0.5))
        st.plotly_chart(style_plotly(fig_pie), use_container_width=True)
        card_close()

    # Tabel (card)
    card_open("🔍 Cuplikan Data Mentah (Trip-level)")
    st.dataframe(df_f.head(50), use_container_width=True)
    card_close()

    card_open("📅 Data Harian (Agregasi)")
...
# ============================================================
    ...
    # KPI prediksi dibikin card juga (konsisten)
    a1, a2, a3 = st.columns(3)
    with a1:
        st.markdown(
            f"""
            <div class="kpi-card">
                <div class="kpi-title">Rata-rata Prediksi</div>
                <div class="kpi-value">{pred_avg:.0f}</div>
            </div>
            """,
            unsafe_allow_html=True
        )
    with a2:
        st.markdown(
            f"""
            <div class="kpi-card">
                <div class="kpi-title">Prediksi Minimum</div>
                <div class="kpi-value">{pred_min:.0f}</div>
            </div>
            """,
            unsafe_allow_html=True
        )
    with a3:
        st.markdown(
            f"""
            <div class="kpi-card">
                <div class="kpi-title">Prediksi Maksimum</div>
                <div class="kpi-value">{pred_max:.0f}</div>
            </div>
            """,
            unsafe_allow_html=True
        )
    ...
    card_open("📊 Grafik Prediksi Penumpang")
    fig_fc = px.line(fc, x="tanggal", y="prediksi_penumpang")
    fig_fc.update_layout(height=420)
    st.plotly_chart(style_plotly(fig_fc), use_container_width=True)
    card_close()

    card_open("🚨 Indikasi Lonjakan")
...
# ============================================================
    ...
    card_open("📌 Grafik Aktual (Total Penumpang Harian)")
    fig_act = px.line(daily_f, x="tanggal", y="total_penumpang")
    fig_act.update_layout(height=360)
    st.plotly_chart(style_plotly(fig_act), use_container_width=True)
    card_close()

    card_open("📎 Daftar File di outputs/")
...
# ============================================================
    ...
    card_open("ℹ️ Informasi Dashboard")
    st.markdown(
        """
