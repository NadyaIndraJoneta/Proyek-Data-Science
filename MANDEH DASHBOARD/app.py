...
def detect_date_col(df: pd.DataFrame) -> Optional[str]:
	...
	# Deteksi isi yang bisa diparse datetime
	best = None
	best_score = 0
	for c in df.columns:
		try:
...
def detect_numeric_col(df: pd.DataFrame, keywords: List[str]) -> Optional[str]:
	...
	# Fallback: numeric varians terbesar
	if numeric_cols:
		variances = {}
...
# ============================================================
with st.sidebar:
	# FIX: jangan pakai use_container_width (biar aman Streamlit versi lama)
	if safe_exists(LOGO_PATH):
		st.image(LOGO_PATH, use_column_width=True)

	st.markdown("## 🛥️ Dashboard Mandeh")
	st.caption("Prediksi Kunjungan Wisata Laut (XGBoost)")
...
if page == "Home":
	...
	c1.metric("Total Penumpang", kpi_format(total_penumpang))
	c2.metric("Total Trips", kpi_format(total_trips))
	c3.metric("Rata-rata Penumpang/Hari", f"{avg_penumpang_harian:.0f}")
	c4.metric("Rata-rata Trips/Hari", f"{avg_trips_harian:.1f}")
